...
module.exports = Observable.extend({
  ...
  getBin: function (req, res, next) {
   ...
   getAllSandbox: function (req, res, next){
       ...
       function validateCss(css){ //result[0].css
       ...
       function validateHtml(record) { //result[i].html
           w3cvalidator.validate({
               ...
               callback: function (response) {
                   console.log("Results validate HTML");
                   console.log(response);
                   validateCss(record.css);

                   /**result[tmpIndex].validateHtml = response.messages;
                   // depending on the output type, res will either be a json object or a html string
                   tmpIndex++;
       ...
       date = date + "%";
        _this.models.bin.getAllSansbox(date, function (err, result) { //result is the content of the query mysql to table sandbox
            ...
            //validateHtml(result[0]);
            validateCss(result[0].css);
            //with this line maybe I resolve javascript validation ==> only high level ciò


            //TODO surround operation esprima.parse in a try catch block ==> to catch possible runtime exception
            /**for (var i = 0; i < result.length; i++) {
                console.log(esprima.parse(result[i].javascript, { tolerant: true, loc: true }));
            }*/
