  ...
  getBin: function (req, res, next) {
    ...
    getStudentLogin: function (req, res, next) {
        ...
        if (req.session.student) {
            ...
            var params = moment().format('YYYY-MM-DD') + "_" + studentData.name + "_" + studentData.surname + "_" + studentData.matricola + "%";
            ...
            _this.models.bin.getAllStudentRecord(params, function (err, result) { //result is the content of the query mysql to table sandbox
                ...
                console.log("risultati"); //return updated row ==> perfect
                console.log("-" + result + "-");
                if (err) {
                    return next(err);
                }
                ...
                if (result.length) {
                    var urlOldExam = statik + "/" + result[0].url + "/" + result[0].revision + "/edit";
                    var loadJs = false;
    ...
    updateBinCompleteExam: function (req, res, next) {
        var _this = this;
        var urlExam = req.body.urlExam;
        var newUrlExam = urlExam+"_finish";
        var examRevision = req.body.examRevision;
        var params = {
            urlExam: urlExam,
            newUrlExam : newUrlExam,
            examRevision : examRevision
        };

        _this.models.bin.updateBinCompleteExam(params, function (err, result) { //result is the content of the query mysql to table sandbox
            console.log("errori");
            console.log("-"+err+"-"); //return null
            console.log("risultati"); //return updated row ==> perfect
            console.log("-"+result+"-");
    ...
    getFinishStudentsToday: function (req, res, next) {
        if (req.session.professor) {
            var _this = this;
            ...
            _this.models.bin.getAllFinishStudents(date, function (err, result) { //result is the content of the query mysql to table sandbox
                ...
                //now validate html call in your callback function the function for validate css, and after javascript and so on with mocha and chai
                for (var i = 0; i < result.length; i++) {
                    var urlArray = result[i].url.split("_");
                    result[i].name = utils.decodeSpecialChars(urlArray[1]);
                    result[i].surname = utils.decodeSpecialChars(urlArray[2]);
                    result[i].matricola = urlArray[3];
                }
...
//           console.log(urlArray);
           ...
           var statik = _this.helpers.urlForStatic('', req.secure);
           var examUrl = statik+"/"+exams[examIndex].url+"/"+exams[examIndex].revision+"/edit";
...
//                   delete require.cache['c:\\Users\\Trappola\\WebstormProjects\\myJsbin\\test\\myjsbin\\testExam2.js'];
    ...
    getDataStudentLogin:function(req, res){
        res.json(studentData.getDataStudent());
        studentData.setNewStudent(0);
    },
    ...
    setDataClockClassromm: function (req,res, next){
        //control if the access is allowed from this request
        if (req.session.professor) {
            //assign field of the body in my variable with a simple conversion
            var newDurationTest = req.body.inputDurationTest * MINUTE2MILLIS;
            var newDurationOverTIme = req.body.inputDurationOverTimeTest * MINUTE2MILLIS;

            res.json(clockAula.setDataClockAula(newDurationTest, newDurationOverTIme));
        } else {
            res.render("errorPage", {
                serviceIsNotAccessible : true
            });
    ...
    getDataQuestionNumber: function(req, res, next){
        res.json(numberQ.getQuestionNumber());
    },

    setDataQuestionNumber: function (req,res, next){
        //control if the access is allowed from this request
        if (req.session.professor) {
            //assign field of the body in my variable with a simple conversion
            var newQuestionP = req.body.inputNumberQuestionP;
            var newQuestionT = req.body.inputNumberQuestionT;

            res.json(numberQ.setQuestionNumber(newQuestionP, newQuestionT));
        } else {
            res.render("errorPage", {
                serviceIsNotAccessible : true
            });
    ...
    setVoteWeight: function (req,res, next){
        //control if the access is allowed from this request
        if (req.session.professor) {
