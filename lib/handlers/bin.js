      ...
      _this.models.bin.create(params, function (err, result) {
        if (err) {
         ...
         _this.completeCreateBin(result, req, res, next);
          ...
          var urlArray = params.url.split("_");
          var matricola = urlArray[3];
          var urlFile = studentData.getUrlFileStudent();

          var completeURLFile  = urlFile + matricola +".json";
          var content = fs.readFileSync(completeURLFile, "utf8");
          var obj = JSON.parse(content);

	  if(result.html){
	  	obj.question[Jsbinchecked - 1].html = params.html.split("\n");
          }
          if(result.css){
          	obj.question[Jsbinchecked - 1].css = params.css.split("\n");
          }
          if(result.javascript){
          	obj.question[Jsbinchecked - 1].javascript = params.javascript.split("\n");
	  }
	  
          var json = JSON.stringify(obj, null, 2);
          fs.openSync(completeURLFile, "w");
          fs.appendFileSync(completeURLFile, json);
          /*******************************************************************/
      });
    });
  },
