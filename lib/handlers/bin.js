  ...
  getCustom: function (req, res, next) {
    ...
    getBin: function (req, res, next) {
        ...
        function validateStudent(student, url){
            ...
            console.log("DATA "+urlArray[0]);
            console.log("NOME "+urlArray[1]);
            console.log("COGNOME "+urlArray[2]);
            console.log("MATRICOLA "+urlArray[3]);
    ...
    updateBinCompleteExam: function (req, res, next) {
        ...
        console.log("urlExam in updateBinCompleteExam "+urlExam);
        var newUrlExam = urlExam+"_finish";
        console.log("newurlExam in updateBinCompleteExam "+newUrlExam);
        var examRevision = req.body.examRevision;
        console.log("examRevision in updateBinCompleteExam "+examRevision);
        ...
        _this.models.bin.updateBinCompleteExam(params, function (err, result) { //result is the content of the query mysql to table sandbox
            console.log("errori");
            console.log("-"+err+"-"); //return null
            console.log("risultati updateBinCompleteExam"); //return updated row ==> perfect
            console.log("-"+JSON.stringify(result)+"-");
            if (err) {
                return next(err);
            }
    ...
    getFinishStudentsToday: function (req, res, next) {
        if (req.session.professor) {
            ...
            _this.models.bin.getAllFinishStudents(date, function (err, result) { //result is the content of the query mysql to table sandbox
                ...
                console.log("getFinishStudentToday result "+JSON.stringify(result));
                //now validate html call in your callback function the function for validate css, and after javascript and so on with mocha and chai
                for (var i = 0; i < result.length; i++) {
                    var urlArray = result[i].url.split("_");
                    console.log("getFinishStudentToday url "+result[i].url);
                    ...
                    result[i].postazione = urlArray[4];
                }
...
//                   delete require.cache['c:\\Users\\Trappola\\WebstormProjects\\myJsbin\\test\\myjsbin\\testExam2.js'];
    ...
    getDataStudentEnd:function(req, res){
        ...
        if(strUrlExam){
            var urlArray = strUrlExam.split("_");
            dataToSend.name = urlArray[2];
            dataToSend.surname = urlArray[3];
            dataToSend.matricola = urlArray[4];
            dataToSend.postazione = urlArray[5];
            res.json(dataToSend);
        }
        else{
            res.json({
    ...
    this.validateBin(params, function (err) {
      ...
      _this.models.bin.create(params, function (err, result) {
        if (err) {
          ...
          console.log("LA MERDA DI PARAMETRI "+JSON.stringify(result, null, 2));
