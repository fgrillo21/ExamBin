...
var testConfig = require("../../test/myjsbin/testConfig");
//var Mocha = require("mocha");
//var mocha = new Mocha();
//mocha.addFile("test/myjsbin/testExam");
...
//                   console.log(response);
       ...
       function parseUrl(url){
           ...
           exams[examIndex].name = urlArray[1];
           exams[examIndex].surname = urlArray[2];
           ...
           //exams[examIndex].url = "<a href='"+statik+"/"+exams[examIndex].url;
       }
       ...
       //TODO change this assignement with a field of the body
       var date = "2014-10-01";
       date = date + "%";
        _this.models.bin.getAllSansbox(date, function (err, result) { //result is the content of the query mysql to table sandbox
            if (err) {
                return next(err);
            }
            ...
            //TODO decide how to manage insert of new field into result (like validateHtml and validateCSS) with special index or calling in serial way function for validation

            //now validate html call in your callback function the function for validate css
            /*if (exams.length){
            ...
            //customValidationMochaChai("ciao");
            var cheerio = require('cheerio');
            var $ = cheerio.load(exams[2].html);
            $("head").append("<style>\n"+exams[2].css+"\n</style>");
            $("body").append("<div id='mocha'></div>");
            $("body").append("<script src='https://ajax.googleapis.com/ajax/libs/jquery/1.11.0/jquery.min.js'></script>");
            $("body").append("\n<script>\n  var y = $('body').length; var x = 11; \n $(document).ready(function(){ \n x= $('p').length; \n }); function zeta() { return 3;} </script>");

            $("body").append("\n<script src='../../test/myjsbin/mocha.js'></script>");
            $("body").append("\n<script>mocha.setup('bdd');</script>");
            $("body").append("\n<script src='../../test/myjsbin/chai.js'></script>");
            $("body").append("\n<script src='../../test/myjsbin/plainTestExam.js'></script>");

            //teoricamente con questo file faccio andare i test
            $("body").append("\n<script src='../../test/myjsbin/proveJs.js'></script>");
            exams[2].html = $.html();
            console.log(exams[2].html);

            var jsdom = require("jsdom");
            var doc   = jsdom.jsdom(exams[2].html, null, {
                features: {
            ...
            window.addEventListener('load',  function(){
                ...
                console.log(window.borghi);
                console.log("sono no curioso di più");
                console.log(window.$("li.passes").text());
                console.log(window.$("li.failures").text());
                console.log(window.$("li.duration").text());
                //window.mocha.run();
                console.log(window.tested);
                console.log(window.passed.length + ' Tests Passed');
                window.passed.forEach(function(testName){
                 console.log('Passed:', testName);
                 });

                 console.log("\n"+window.failed.length + ' Tests Failed');
                window.failed.forEach(function(testName){
                 console.log('Failed:', testName);
                 });
                ...
                /*var $ = window.$;
                console.log("Puro spettacolo");
                var colorValue = $("p").css("color");
                console.log("color");
                console.log(colorValue);
                console.log("value of variable y");
                console.log(window.y);
                console.log("value of variable length");
                console.log(window.x);
                console.log("value of variable length jquery");
                console.log($('p').length);
                console.log("value of function z");
                console.log(window.zeta());
                res.json(exams);*/
            });
            ...
            /*jsdom.env({
                ...
                src:[" var y = 7; var length= $('p').length;"],
            ...
            //TODO surround operation esprima.parse in a try catch block ==> to catch possible runtime exception
            /*for (var i = 0; i < result.length; i++) {
                esprima.parse(result[i].javascript, { tolerant: true, loc: true });
            }*/
        });
    },
    ...
    this.protectVisibility(req.session.user, bin, function(err, bin) {
      ...
      var done = function () {
        ...
        res.render('index', {
          ...
          timeout: clockAula.getTimeout() //TODO try and after commit changes
        });
      };
