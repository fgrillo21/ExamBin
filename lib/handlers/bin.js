...
module.exports = Observable.extend({
  ...
  getBin: function (req, res, next) {
   ...
   getAllSandbox: function (req, res, next){
       ...
       var date = "2014-09-29";
       date = date + "%";
        _this.models.bin.getAllSansbox(date, function (err, result) {
            if (err) {
                return next(err);
            }

            tmpIndex = 0;
            //w3cjs.setW3cCheckUrl("http://jigsaw.w3.org/css-validator/validator");
            /**for (i = 0; i < result.length; i++){
                var results = w3cjs.validate({
                    ...
                    callback: function (response) {
                        console.log("Vediamo se è veramente quasi magia E quasi magia jhonny");
            ...
            var results = w3cvalidator.validate({
                ...
                callback: function (res) {
                    console.log("Speriamo in sti risultati cio");
                    console.log(res);
                    // depending on the output type, res will either be a json object or a html string
                },
