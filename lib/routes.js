...
module.exports = function (app) {
  ...
  app.post('/createFileJson', function(req, res) {
    if (req.session.professor) {
      ...
      console.log("data "+currentDateExam);
      var dirResult = './views/result/';
      var dirResultToday = './views/result/'+currentDateExam+"_P"+numberExam;
       ...
       * per chiarezza il nome della cartella coincide con la data dell'esame */
      if (!fs.existsSync(dirResultToday)) {
        ...
        var filename = [];
        var list = fs.readdirSync(dirResult);
        list.forEach(function (file) {
  ...
  app.post('/saveValueTextarea', function(req, res){
    ...
    fs.openSync(completePath, "w");
    fs.appendFileSync(completePath, json);
    /*******************************************************************/
  });

  app.post('/setIdJsbinChecked', function(req, res){
    if(req.session.student){
      var id = req.body.id;
