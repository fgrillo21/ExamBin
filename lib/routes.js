...
module.exports = function (app) {
  ...
  app.get('/professor', function(req, res) {
    console.log("IP");
      ...
      console.log(ip); //todo i need to verify this ip to confirm access
      console.log(__dirname);
      req.session.professor = true;
      var statik = sandbox.helpers.urlForStatic('', req.secure);
      //vediamo cosa cè dentro l'oggetto req
      //console.log(req);
    res.render('controlPanelProf',{
        static: statik,
        timeout: 60 //perfect IT WORKS
    });
  });
    ...
    app.post('/correction/uploadFileCustom', function(req, res) {
        console.log(req.files.fileMochaUpload);

        var tmp_path, target_path, config_path;
        var dataToAppend;
  ...
  app.post('/uploadFileCustom', function(req, res){
    ...
    var tmp_path, target_path;
      ...
      if (req.files.fileHtmlUpload.name !== '' && req.files.fileHtmlUpload.size !== 0){
        // get the temporary location of the file
        tmp_path = req.files.fileHtmlUpload.path;
        // set where the file should actually exists - in this case it is in the "images" directory
        target_path = './views/' + "default.html";
        // move the file from the temporary location to the intended location
...
//                res.send('File uploaded to: ' + target_path + ' - ' + req.files.fileUploaded.size + ' bytes');
      ...
      if (req.files.fileCssUpload.name !== '' && req.files.fileCssUpload.size !== 0){
          // get the temporary location of the file
          tmp_path = req.files.fileCssUpload.path;
          // set where the file should actually exists - in this case it is in the "images" directory
          target_path = './views/' + "default.css";
          // move the file from the temporary location to the intended location
          fs.rename(tmp_path, target_path, function(err) {
              if (err) throw err;
              // delete the temporary file, so that the explicitly set temporary upload dir does not get filled with unwanted files
      ...
      if (req.files.fileJavascriptUpload.name !== '' && req.files.fileJavascriptUpload.size !== 0){
          // get the temporary location of the file
          tmp_path = req.files.fileJavascriptUpload.path;
          // set where the file should actually exists - in this case it is in the "images" directory
          target_path = './views/' + "default.js";
          // move the file from the temporary location to the intended location
          fs.rename(tmp_path, target_path, function(err) {
              if (err) throw err;
              // delete the temporary file, so that the explicitly set temporary upload dir does not get filled with unwanted files
      ...
      function sendData(){
          res.send('File Exam Upload with Sucess');
      }
  });
