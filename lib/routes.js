...
var numberQ = require('./myjsbin/number');
var jsonFile = require('../question.json');
...
//      if (ip === ipProfMachine) {
        if (ip) {
            ...
            var question = numberQ.getQuestionNumber();
            res.render('controlPanelProf', {
                  static: statik,
                  numQuestionP: question.numQuestionP,
                  numQuestionT: question.numQuestionT,
...
//                        res.send('File uploaded to: ' + target_path + ' - ' + req.files.fileMochaUpload.size + ' bytes');
    ...
    app.post('/clearQuestionFile', function(req, res) {
      if (req.session.professor) {
        ...
        var click = numberQ.getClick();
        var i;
        for(i=1; i<click; i++){
          var nameDir = 'q' + i;
          deleteFolderRecursive('./views/question/'+nameDir);
        }
        /* risetto il click al valore di default, in modo che all'inizio della nuova sessione d'esame il conteggio delle cartelle riparta da 1 */
        numberQ.setClick(1);

        res.json({
          ok: true,
          message: "Cartelle cancellate"
        })
      }
      else {
        res.render("errorPage", {
  ...
  app.post('/cloneDivQuestion', function(req, res){
    if (req.session.professor) {
      res.json({
        ok : true,
        message: 'Clone effettuato con successo'
      });
    }
    else {
      res.render("errorPage", {
  ...
  app.post('/createFileJson', function(req, res) {
    if (req.session.professor) {
      ...
      console.log("DATTTAAAAAAAAAAA:  ", json);
      ...
      res.json({
        ok: true,
        message: "FILE JSON CREATI CON SUCCESSO",
        path: pathFileJson,
        name: nameFile
      })
    }
  ...
  app.post('/uploadFileCustom', function(req, res){
      if (req.session.professor) {
          /* recupero il valore di default (settato a 1), che rappresenta il numero di volte in cui il pulsante aggiorna viene cliccato */
          var click = numberQ.getClick();
          console.log("CLICK: "+click);

          var inputFileHtml = req.files.fileHtmlUpload.path;
          var inputFileCss = req.files.fileCssUpload.path;
          var inputFileJavascript = req.files.fileJavascriptUpload.path;

          var pathFileJson = './views/question/q' + click + '.json';
          var obj = JSON.parse(fs.readFileSync(pathFileJson, "utf8"));

           /* inizializzo i campi html, css e javascript con gli oppurtuni valori */
          var result = JSON.stringify(obj, function replacer(k, v) {
            if (k=== "html") {
              /* il controllo sulla size non è più !== 0 ma >0, perché il professore può anche decidere di caricare dei file vuoti (???) */
              if (req.files.fileHtmlUpload.name !== '' && req.files.fileHtmlUpload.size >= 0) {
                var dataFileHtml = fs.readFileSync(inputFileHtml, "utf8");
                console.log("dataFile " + dataFileHtml);
                v = dataFileHtml;
                return v;
              }
            }
            if (k=== "css") {
              if (req.files.fileCssUpload.name !== '' && req.files.fileCssUpload.size >= 0) {
                var dataFileCss = fs.readFileSync(inputFileCss, "utf8");
                console.log("dataFile " + dataFileCss);
                v = dataFileCss;
                return v;
              }
            }
            if (k=== "javascript") {
              if (req.files.fileJavascriptUpload.name !== '' && req.files.fileJavascriptUpload.size >= 0) {
                var dataFileJavascript = fs.readFileSync(inputFileJavascript, "utf8");
                console.log("dataFile " + dataFileJavascript);
                v = dataFileJavascript;
                return v;
              }
            }
            return v;
          });
          fs.writeFile(pathFileJson, result, function (err) {
            if (err) throw err;
            sendData();
          });
      }
    ...
    app.get('/professor/previewFile', function (req, res, next){
      ...
      if (req.session.professor) {
        var tmpPath = __dirname.substr(0, __dirname.length - 3);
        var path = tmpPath + pathModule.sep + 'question.json';
        res.sendfile(path);
      } else {
        res.render("errorPage", {
          serviceIsNotAccessible : true
