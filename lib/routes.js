...
module.exports = function (app) {
  ...
  app.post('/setIdJsbinChecked', function(req, res){
    if(req.session.student){
      var store = studentData.getIdJsbinChecked();
      var find = false;
      var id = req.body.id;
      ...
      if(store.length === 0){
        studentData.setIdJsbinChecked(id);
      }
      else {
        /* vado a cercare nell'array, che contiene gli id dei checkbox, se esiste già un risposta per una determinata domanda, se esiste già
         * significa che lo studente ha cambiato idea ed ha selezionato un'altra domanda, quindi viene rimosso l'id di quella precedentemente
         * selezionata e inserito quello nuovo, altrimenti si fa il normale inserimento */
        for (i = 0; i < store.length && find === false; i++) {
          if (store[i].slice(0, 7) === idslice) {
            find = true;
            idToRemove = i;
          }
        }
        if(find){
          //console.log("store slice " + store[idToRemove].slice(0, 7)+ " == " + idslice + "    i "+idToRemove);
          studentData.removeIdJsbinChecked(idToRemove);
          studentData.setIdJsbinChecked(id);
        }
        else{
          studentData.setIdJsbinChecked(id);
      ...
      store = studentData.getIdJsbinChecked();
      console.log("ARRAYCHECKEDID ["+store+"] len: "+store.length);
      res.json({
        ok : true,
        id: req.body.id
      });
 ...
 //app.get('/getFinishStudent', binHandler.getFinishStudentsToday);
  ...
  app.post('/deliveryExam', function(req, res){
