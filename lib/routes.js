...
module.exports = function (app) {
  ...
  function binParamFromReferer(req, next) {
  ...
  app.get('/teacher/correction', function(req, res) {
    console.log("IP");
    // per recuperare l'ip del chiamante
    var ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress;
    console.log(ip);
  ...
  app.post('/loadValueToJsbin', function(req, res){
    if(req.session.student) {
      var matricola = req.session.student.matricola;
      var tmpPath = __dirname.substr(0, __dirname.length - 3);
      var pathResultToday = './views/result/'+currentExam+"/"+matricola+".json";
      var path = pathResultToday.slice(2);
      var completePath = tmpPath + path;
      var content = fs.readFileSync(completePath, "utf8");
      var obj = JSON.parse(content);
      var id = req.body.id;
      var html = obj.question[id - 1].html;
      var css = obj.question[id - 1].css;
      var javascript = obj.question[id - 1].javascript;

      var urlToSend = './views/result/'+currentExam+"/";
      var pathUrlToSend = urlToSend.slice(2);
      var completeUrlToSend = tmpPath + pathUrlToSend;

      var jsbinChecked = id;
      studentData.setJsbinChecked(jsbinChecked);
      studentData.setUrlFileStudent(completeUrlToSend);

      var targetPathHtml = './views/' + "default.html";
      var targetPathCss = './views/' + "default.css";
      var targetPathJavascript = './views/' + "default.js";
      var i;

      if(obj.question[id-1].hasOwnProperty('html')){
        console.log("HTTTTTTTML");
        fs.writeFileSync(targetPathHtml, '');
        for (i = 0; i < html.length; i++) {
          if (i !== html.length -1)
            fs.appendFileSync(targetPathHtml, html[i]+"\n");
          else
            fs.appendFileSync(targetPathHtml, html[i]);
        }
      }
      ...
      if(obj.question[id-1].hasOwnProperty('css')) {
        fs.writeFileSync(targetPathCss, '');
        for (i = 0; i < css.length; i++) {
          if (i !== css.length -1)
            fs.appendFileSync(targetPathCss, css[i]+"\n");
          else
            fs.appendFileSync(targetPathCss, css[i]);
        }
      }
      ...
      if(obj.question[id-1].hasOwnProperty('javascript')) {
        fs.writeFileSync(targetPathJavascript, '');
        for (i = 0; i < javascript.length; i++) {
          if (i !== javascript.length -1)
            fs.appendFileSync(targetPathJavascript, javascript[i]+"\n");
          else
            fs.appendFileSync(targetPathJavascript, javascript[i]);
        }
      }
  ...
  app.post('/loadQuestion', function(req, res){
    if(req.session.student){
      /* tmpPath: /home/francy/WebStorm/NPA-Exam/ */
      var tmpPath = __dirname.substr(0, __dirname.length - 3);
      /* pathLastFile: ./views/exams/28_07_2015.json */
      /* path: views/exams/28_07_2015.json */
      var path = pathLastFile.slice(2);
      /* completePath: /home/francy/WebStorm/NPA-Exam/views/exams/28_07_2015.json */
      var completePath = tmpPath + path;
  ...
  app.post('/appendFile', function(req, res) {
    if (req.session.professor) {
      var p = './views/exams';
      var q = './views/result';
      /* se non esiste viene creata la directory che conterrà le cartelle di ogni prova d'esame */
      if (!fs.existsSync(q)) {
        createDirectory(q);
      }
      /* se non esiste viene creata la directory che conterrà i file d'esame configurati dal professore */
      if (!fs.existsSync(p)) {
        createDirectory(p);
      }
      else {
        ...
        var nameLastFile = getMostRecentFileName(p);
        pathLastFile = p + nameLastFile;

        if (results) {
          //console.log("Sono presenti dei file");
          res.json({
            ok: true,
          ...
          console.log("Non ci sono file da caricare");
        }
      }
    }
    else {
  ...
  app.post('/createCopy',function(req, res){
    if (req.session.professor) {
      ...
      var pathFileJson = './views/exams/'+nameFile+".json";
      var newName;

      while(fs.existsSync(pathFileJson)) {
        pathFileJson = './views/exams/'+nameFile+"_"+i+"c.json";
        newName = nameFile+"_" +i +"c";
        i++;
      }
  ...
  app.post('/createFileJson', function(req, res) {
    if (req.session.professor) {
      var json = JSON.stringify(req.body, null, 2);
      var nameExam = req.body.name;
      if(nameExam.slice(-1) === "c"){
        if(currentDateExam !== nameExam.slice(10,20)){
          numberExam = 1;
        }
        currentDateExam = nameExam.slice(10,20);
      }
      else{
        if(currentDateExam !== nameExam.slice(-10)){
          numberExam = 1;
        }
        currentDateExam = nameExam.slice(-10);
      }
      var pathFileJson = './views/exams/'+currentDateExam+".json";

      currentExam = currentDateExam+"_P"+numberExam;
      var dir = './views/result/';
      var dirResultToday = './views/result/'+currentExam;

      /* se non esiste viene creata la directory che conterrà il compito consegnato da ogni studente
       * per chiarezza il nome della cartella coincide con la data dell'esame + il numero della prova */
      if (!fs.existsSync(dirResultToday)) {
        createDirectory(dirResultToday);
      }
        ...
        var lastDir = filename.pop();
        var numberlastDir = lastDir.slice(-1);
        numberExam = parseInt(numberlastDir) + 1;
        currentExam = currentDateExam+"_P"+numberExam;
        var newDirResultToday = './views/result/'+currentExam;
        createDirectory(newDirResultToday);
      }
  ...
  /*** Salvataggio valore textarea all'interno del file Json dello studente ***/
  app.post('/saveValueTextarea', function(req, res){
    ...
    var pathResultToday = './views/result/'+currentExam+"/"+matricola+".json";
    var path = pathResultToday.slice(2);
    var urlFile = tmpPath + path;
    var content = fs.readFileSync(urlFile, "utf8");
    var obj = JSON.parse(content);

    obj.question[last - 1].answer = answer.split("\n");
  ...
  /*** Salvataggio risposta multipla all'interno del file Json dello studente ***/
  app.post('/saveRadioChecked', function(req, res){
    if(req.session.student){
      var id = req.body.id;
      var value = req.body.value;
      var idslice = id.slice(0,7);
      var numberQuestion = idslice.slice(-1);
      ...
      obj.question[numberQuestion - 1].options = value;
  ...
  app.post('/confirmStudent', function(req, res) {
    ...
    createFileResultStudent(req.body.matricola);
    ...
    /* ogni file è identificato dalla matricola dello studente in modo da essere univoco */
    function createFileResultStudent(matricola){
      var directoryResultStudent = './views/result/'+currentExam+"/";
      var fileResultStudent = directoryResultStudent+matricola+".json";
      ...
      /* il file di partenza è uguale a quello caricato dal professore prima dell'inizio della prova
       * ad ogni modifica opportata verranno aggiornati i valori */
      fs.openSync(fileResultStudent, "w");
      fs.appendFileSync(fileResultStudent, content);
    }
  });
  ...
  app.get('/professor/correction/file', function (req, res){
    if(req.session.professor){
      var tmpPath = __dirname.substr(0, __dirname.length - 3);
  ...
  //pagina che contiene la lista di domande per l'esame
  app.post('/list', binHandler.updateList);

  app.get('/listPage_:params', function (req, res){
    res.render('listQuestion',null);
  });
  //

  app.get('/getClockAula', binHandler.getClockClassroom);

  app.post('/deliveryExam', binHandler.updateCompleteExam);

  app.post('/deleteExamNotDelivery', function(req, res){
    var ExamNotDelivery = req.body.array;
    var dirResultToday = './views/result/'+currentExam;

    var list = fs.readdirSync(dirResultToday);
    if(ExamNotDelivery) {
      for (var i = 0; i < ExamNotDelivery.length; i++) {
        list.forEach(function (file) {
          var tmp = ExamNotDelivery[i] + ".json";
          if (tmp === file) {
            var filePath = dirResultToday + "/" + file;
    ...
    studentData.clearArrayUrlExamComplete();
  ...
  app.post('/getFinishPage', binHandler.getFinishPage);

  app.get('/finishExam', function (req, res){
    res.render('finishPage',null);
  });

  /* servizio che recupera i dati degli studenti che hanno terminato l'esame */
  app.get('/getFinishStudent', binHandler.getDataStudentEnd);

  app.get('/professor/correction/exampleCustomValidation', function (req, res){
  ...
  app.get('/professor/correction/currentCustomValidation', function (req, res){
    //control if the access is allowed from this request
    if (req.session.professor) {
      var tmpPath = __dirname.substr(0, __dirname.length - 3);
      //this istruction do the MAGIC
  ...
  app.post('/setDataOverTimeHalfHour', binHandler.setDataOverTimeHalfHour);
