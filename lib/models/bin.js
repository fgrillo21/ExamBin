...
var model = {
  ...
  create: function (data, fn) {
    this.store.generateBinId(data.student, 0, function generateBinId(err, id) {
      if (err) {
        return fn(err);
      }
  ...
  isStreaming: function (bin) {
    ...
    getAllFinishStudents: function(params, fn){
      this.store.getAllSandbox(params, fn);
    },
    updateBinCompleteExam: function(params, fn) {
        this.store.updateBinCompleteExam(params, fn);
    },
    getAllExamDate: function(params, fn) {
        this.store.getAllExamDate(params, fn);
    },
    getAllStudentRecord: function(params, fn){
        this.store.getAllStudentRecord(params, fn);
    },
    getLastIdSandbox: function(params, fn){
    this.store.getLastIdSandbox(params, fn);
}
};
