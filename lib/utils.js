...
var student = require('./myjsbin/studentData');
var index_autoincrement = 0;
...
module.exports = {
  ...
  shortcode: function (studentData, id) {
    /**var vowels = 'aeiou',
    ...
    return word; */
        //console.log("DATI RICEVUTIIII "+studentData);
      var word;
      if (studentData) {
          if(id === 0){
            word = moment().format('YYYY-MM-DD') + "_" + studentData.name + "_" + studentData.surname + "_" + studentData.matricola + "_" + studentData.postazione;
          }
          else {
            studentData.id = id;
            word = moment().format('YYYY-MM-DD') + "_" + studentData.name + "_" + studentData.surname + "_" + studentData.matricola + "_" + studentData.postazione+  "_" +studentData.id;
          }
      } else {
          console.log("Se entro in questo ramo è perchè si è loggato il professore");
          word = "prove_professore_"+index_autoincrement;
      }
      index_autoincrement++;
      return word;
  },
  ...
  binUrlLength: function (req) {
    ...
    decodeSpecialChars: function (inputString) {
        var s = inputString;
        while (s.indexOf("-") !== -1){
            s = s.replace("-","%");
        }
    ...
    encodeSpecialChars: function (inputString) {
        var s = inputString;
        s = escape(s);
        while (s.indexOf("%") !== -1) {
            s = s.replace("%", "-");
    ...
    setAutoIncrementIndex: function (startIndex) {
        if (startIndex) {
            index_autoincrement = 0;
        } else {
            index_autoincrement = 0;
        }
        console.log("index "+index_autoincrement);
    }
};
