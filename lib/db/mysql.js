...
module.exports = utils.inherit(Object, {
  ...
  generateBinId: function (student, attempts, fn) {
    attempts = attempts || 1;
    var mysql = this;
    ...
    function tryurl(triesLeft, fn) {
      ...
      var id = utils.shortcode(student);
    ...
    if (attempts <= 10) {
      tryurl(3, function (err, id) {
        if (err) {
          ...
          mysql.generateBinId(student, attempts + 1, fn);
        }
      });
    } else {
  ...
  getAllSandbox:function (params, fn){
    var sql = templates.getAllSandbox;
   ...
   updateBinCompleteExam:function (params, fn){
       var sql = templates.updateBinCompleteExam;
       var urlExam = params.urlExam;
       var newUrlExam = params.newUrlExam;
       var examRevision = params.examRevision;
       this.connection.query(sql, [newUrlExam , urlExam, examRevision], fn);
   },
    getAllExamDate:function (params, fn){
        var sql = templates.getAllExamDate;
        this.connection.query(sql, null , function (error, result) {
            if (error || !result) {
                return fn(error || { notfound: true });
            }
            fn(null, result);
    ...
    getAllStudentRecord:function (params, fn){
        var sql = templates.getAllStudentRecord;
        this.connection.query(sql, [params], function (error, result) {
            if (error || !result) {
                return fn(error || { notfound: true });
            }
    ...
    getLastIdSandbox:function (params, fn){
        var sql = templates.getLastIdSandbox;
        this.connection.query(sql, null, fn);
    }
});
