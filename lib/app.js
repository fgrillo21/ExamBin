...
var clock = require("./myjsbin/timer");
...
clock.setExamUrl(app.get("url full"));
...
app.connect = function (callback) {
  ...
  app.configure(function () {
    ...
    /** TODO fare attenziona ad usare csrf, occhio alla riga sottostante, mi fà paura **/
    //app.use(middleware.csrf({ ignore: ['/',/^\/api\//, /^\/hooks\//, /^\/subscribe\//] }));
    app.use(middleware.csrf({ ignore: ['/','/deleteExamNotDelivery','/saveValueTextarea','/getFinishPage','/saveIdRadioChecked','/loadValueToJsbin','/loadQuestion','/list','/setDataOverTimeOne','/setDataOverTimeFive', '/setDataOverTimeHalfHour','/createCopy','/setupLoginStudent','/getValueFileJson','/appendFile','/professor/uploadExamTextFile','/createFileJson','/clearQuestionFile','/correction/setWeightValidation','/correction/uploadFileCustom','/setClockAula','/deliveryExam','/setDataClockAula','/uploadFileCustom','/confirmStudent',/^\/api\//, /^\/hooks\//, /^\/subscribe\//] }));
